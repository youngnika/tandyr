    ...
    window.customElements.define("c-cart", class extends HTMLElement {
        ...
        connectedCallback() {
            ...
            this.$form.addEventListener('submit', function(e) {
                ...
                // Сбор информации о продуктах в корзине
                let cartItems = JSON.parse(localStorage.getItem('cart')) || [];
                let productsInfo = cartItems.map(item => `Продукт: ${item.name}, Количество: ${item.count}, Цена за шт: ${item.price}`).join('\n');
        
                // Составляем сообщение
                let message = `Заявка с сайта\nИмя: ${name}\nНомер телефона: ${phone}\nАдрес: ${address}\n\nПродукты:\n${productsInfo}`;
        
                // Параметры API Telegram
                const TOKEN = '7467074329:AAExsoWs1W2-ShC2Tuy7J8ekaJ1xP2e4_7Y';
                const CHAT_ID = '-1002217309068';
                ...
                .then((res) => {
                    console.log('Сообщение отправлено', res.data);
                    // Например, можно показать сообщение об успехе
                })
                .catch((err) => {
        ...
        _bindControls() {
            this.$form.addEventListener("submit", t=>{
                this.dispatchEvent(new CustomEvent("submit",{
                    ...
                    detail: {
                        ...
                        name: this.$form.querySelector('[name="address"]').value,
            ...
            this.$adds.forEach(t=>{
                t.addEventListener("click", ()=>{
                    ...
                    let o;
                    n ? (o = document.createElement("img")).src = n : (o = document.createElement("c-icon")).setAttribute("name", "good"),
                    ...
                    o.style.transition = "top .5s ease-in-out, left .5s ease-in-out",
                    ...
                    o.style.left = `${l.right - (l.width / 2 + c.width / 2)}px`;
        ...
        _getElements() {
            ...
            this.fab ? (this.$fab = document.createElement("c-cart-fab"),
            this.shadowRoot.appendChild(this.$fab)) : this.$fab = document.querySelector('.cart-fab:not([style*="display: none"])'),
            ...
            this.$sum = this.shadowRoot.querySelector("#summary c-aninumber"),
            this.$empty = this.shadowRoot.querySelector("#empty"),
            this.$cartFields = [...this.shadowRoot.querySelectorAll('input[name="cart"]'), ...document.querySelectorAll('input[name="cart"]')],
            this.$toggles = [...this.shadowRoot.querySelectorAll(".cart-toggle"), ...document.querySelectorAll(".cart-toggle")],
            this.$counters = [...this.shadowRoot.querySelectorAll(".cart-counter"), ...document.querySelectorAll(".cart-counter")],
            this.$adds = [...document.querySelectorAll(".menu-btn")],
            this.$form = this.shadowRoot.querySelector("form")
        }
        addItem(t) {
            let e = !1;
            this.items.forEach((n,i)=>{
                t.name === n.name && (this.items[i].count += t.count,
                e = !0)
            }
            ),
            e || (this.items.push(t),
        ...
        toggle() {
            if (this.$cart.classList.contains("closed")) {
                const color1 = "#B02E0C";
                const color2 = "#F7BD28";
                this.$header.style.background = `linear-gradient(to right, ${color1}, ${color2})`,
                this.$submit.style.background = `linear-gradient(to right, ${color1}, ${color2})`
            }
        ...
        _render() {
            ...
            this.items.forEach((t,e)=>{
                ...
                n.addEventListener("change", t=>{
                    0 === t.detail ? this.removeItem(e) : (this.items[e].count = t.detail,
                    this._saveCart(),
                    this._updateSum(),
                    this._updateCounter()),
                    this.dispatchEvent(new CustomEvent("change",{
...
, function(t, e) {
    ...
    window.ShadyCSS && window.ShadyCSS.prepareTemplate(n, "c-cart"),
    window.customElements.define("c-cart-item", class extends HTMLElement {
        connectedCallback() {
        ...
        _bindControls() {
            ...
            this.$input.addEventListener("input", ()=>{
                const t = parseInt(this.$input.value, 10) || 0
                  , e = this.count;
                this.count = this.$input.value = t > 999 || t < 1 ? this.count : t,
                e !== this.count && this._render()
            }
            ),
